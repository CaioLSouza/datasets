...
This version intentionally follows the methodology of the first code supplied
by the user:
...
Earnings growth is the residual. Component histories and period returns are
compounded independently, exactly as in the original script.
"""
...
def build_annual(
    ...
    rows = []
    for name, data in daily.items():
        data = data.loc[analysis_start:]
        for year, group in data.groupby(data.index.year):
            row = independently_compound(group)
            rows.append(
                {
                    "series": name,
                    "year": int(year),
...
def build_periods(daily: dict[str, pd.DataFrame]) -> pd.DataFrame:
    rows = []
    for name, data in daily.items():
        end = data.index.max()
        periods = {
...
def save_workbook(
    ...
    methodology = pd.DataFrame(
        [
            ...
            {
                "item": "Aggregation",
                "value": "Each component compounded independently, matching the original code",
            },
            {
                "item": "Period residual",
                "value": "Reported because independently compounded components are not additive",
            },
        ]
    )
...
def run(args: argparse.Namespace) -> None:
    ...
    annual = build_annual(daily, args.analysis_start)
    periods = build_periods(daily)
    ...
    validation = {
        ...
        "note": (
            "Period compounding residual is expected because components are "
            "compounded independently, matching the original methodology."
        ),
    }
